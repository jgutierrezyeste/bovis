   ...
   **/
   function showNavigationHeader($options = []) {
      ...
      if (empty($withtemplate)
         ...
         echo "<td><a id='goToList' href=\"".$glpilisturl."\" title='$glpilisttitle'>";
         ...
         echo "<td style='font-weight: bold; font-size: 16px; color:#0e52a0; '>";
         if (!self::isLayoutWithMain() || self::isLayoutExcludedPage()) {
            echo " ".$name." ";
         }         
         //if(options['projecttask_id']){
             echo "<input id='identificador' type='hidden' value='{$ID}' />";
             echo "<div id='tipo' style='background-color:#f8f7f3; border:none; display: inline-block; margin-right: 4px; margin-left: 4px;'></div>";
         //}
         echo "</td>";
