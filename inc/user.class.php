...
$titlerand = mt_rand();
echo "<tr class='tab_bg_1'><td><label for='dropdown_usertitles_id$titlerand'>Puesto</label></td><td>";
...
if (empty($ID)) {
 ...
 echo "</td></tr>";
 echo "<tr class='tab_bg_1'><td><label for='dropdown__profiles_id$profilerand'>" .  __('Supplier') . "</label></td><td>";
 $options['name']='supplier_id';
 Dropdown::show('Supplier',$options);
 echo "</td></tr>";
...
echo "</table></div>";
   ...
   **/
   static function dropPictureFiles($picture) {
    ...
    /*
     * para saber si es super-administrador o administrador
     */
    function isMaster(){
            global $DB;

            $query ="SELECT profiles_id as profile FROM glpi_users WHERE id=".Session::getLoginUserID();

            $result=$DB->query($query);
            $id=$DB->fetch_array($result);

            if($id['profile']=='3' || $id['profile']=='4' || $id['profile']=='16'){
                    return true;
            }else{ return false;}
    }
