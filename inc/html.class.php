...
class Html {
   ...
   static function convDate($time, $format = null) {
      ...
      switch ($format) {
         ...
         case 3 :
            ...
            //$iMes = date("n", $fechaComoEntero);
            //$iAnio = date("Y", $date);
            $iMes = $date->format("n")-1;
            $fecha = $meses[$iMes]."/ ".$date->format("Y");
      }
   ...
   **/
   static function includeHeader($title = '', $sector = 'none', $item = 'none', $option = '') {
      ...
      echo Html::css('/lib/jqueryplugins/datatables.min.css');
      ...
      echo Html::script('lib/jqueryplugins/datatables.min.js');
      echo Html::script('lib/jqueryplugins/dataTables.buttons.min.js');
      echo Html::script('lib/jqueryplugins/pdfmake.min.js');
      echo Html::script('lib/jqueryplugins/vfs_fonts.js');
      echo Html::script('lib/jqueryplugins/jszip.min.js');
      echo Html::script('lib/jqueryplugins/buttons.html5.min.js');      
      
      
      // Gráficas y estadísticas
      echo Html::script('lib/jqueryplugins/jquery.flot.min.js');
      echo Html::script('lib/jqueryplugins/jquery.flot.pie.min.js');
      echo Html::script('lib/jqueryplugins/excanvas.min.js');
      echo Html::script('lib/jqueryplugins/jquery.flot.time.min.js');
      echo Html::script('lib/jqueryplugins/jquery.flot.symbol.min.js');
   ...
   **/
   static function footer($keepDB = false) {
      ...
      if (!empty($CFG_GLPI["founded_new_version"])) {
         ...
         $latest_version = "<a href='http://www.fotex.es' target='_blank' title=\"".
                              __s('Puede buscarla en el site www.fotex.')."\"> ".
   ...
   **/
   static function showDateField($name, $options = []) {
      ...
      switch ($_SESSION['glpidate_format']) {
         case 1 :
            $p['showyear'] ? $format='dd-mm-yyyy' : $format='dd-mm';
            break;

         case 2 :
            $p['showyear'] ? $format='mm-dd-yyyy' : $format='mm-dd';
            break;

         default :
            $p['showyear'] ? $format='yyyy-mm-dd' : $format='mm-dd';
      }
   ...
   **/
   static function showColorField($name, $options = []) {
    ...
    static function showDateMounth ($name, $value, $min, $max, $rand, $required) {
        global $CFG_GLPI;
        
        if ($value != '') {
            $fechaEntero = strtotime($value);            
        }else{
            $fechaEntero = getdate();
        }
        $dia  = date('d', $fechaEntero);
        $mes  = str_pad(date('m', $fechaEntero), 2, "0", STR_PAD_LEFT);
        ...
        $meses = array('Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic');
        $output = "<select id='".$name."_MES' value='".$mes."' onchange='modifica();' style='height: 25px; width: 6em; border-radius: 4px; margin-right: 2px;'>";
        for ($i=1; $i<=sizeof($meses); $i++){
            if (str_pad($i, 2, "0", STR_PAD_LEFT)==$mes) {
                $output.="<option value='".str_pad($i, 2, "0", STR_PAD_LEFT)."' selected>".$meses[$i-1].'</option>';
            }else{
                $output.="<option value='".str_pad($i, 2, "0", STR_PAD_LEFT)."'>".$meses[$i-1].'</option>';
            }
	}
        $output.="</select>";       
        $output.="<input id='".$name."_ANIO' value='".$anio."' onchange='modifica();' type='number' min='".$min."' max='".$max."' step='1' {$req} style='height: 25px; width: 6em; border-radius: 4px; margin-right: 2px;'/>";
        $output.="<input id='".$name.$rand."' name='".$name."' type='hidden' value='".$anio."-".$mes."-".$dia."' />";
        
        $js = "$(function(){";
        
        $js .= "
            $('#".$name."_MES').on('click', function (){
               modifica();
            });
            $('#".$name."_MES').on('blur', function (){
               modifica();
            });            
            $('#".$name."_ANIO').on('click', function (){
                modifica();
            });
            $('#".$name."_ANIO').on('blur', function (){
                modifica();
            });            
            function modifica() {
                var anio = $('#".$name."_ANIO').val();
                var mes = $('#".$name."_MES').val();
                $('#".$name.$rand."').val(anio+'-'+mes+'-'+'01');            
            };            
            })";
        $output.=Html::scriptBlock($js);
        echo $output;
...
//                                  dateFormat: 'M yy',
//                                  buttonText: '<i class=\'fa fa-calendar\'></i>'}";	
//                $js .= "$('.ui-datepicker-calendar').css('display','none');";
//        }
      ...
      if ($p['hideday']) {
		  $js .= ",changeMonth:true,changeYear:true,onClose:function(dateText, inst){
	 ...
	 //}
      ...
      $output = Html::scriptBlock($js).$output;
...
//                               return {results: data.results};
   ...
   **/
   static function submit($caption, $options = []) {
      ...
      if (!isset($options['class'])) {
		...
		switch ($options['name']) {
			CASE "add_nueva_especialidad":
				$options['class'] = 'boton_add';
				break;					
			CASE "add":
				$options['class'] = 'boton_add';
				break;				
			CASE "update":
				$options['class'] = 'boton_grabar';
				break;
			CASE "purge":
				$options['class'] = 'boton_borrar';
				break;
			CASE "delete":
				$options['class'] = 'boton_borrar';
      ...
      if (isset($options['confirm']) || $options['name']=="purgue" || $options['name']=="delete") {
         if (!empty($options['confirm'])) {
            $confirmMessage = $options['confirm'];
            $confirmAction  = '';
            if (isset($options['confirmaction'])) {
	  ...
	  switch ($options['name']) {
		...
		case "delete":
			$valor = " ";
			$titulo = "Tirar a la papelera";
			break;		
		default: 
			$valor = $caption;
			break;
	  }
	  return sprintf('<input type="submit" value="%s" title="%s" %s />',
				$valor, $titulo, Html::parseAttributes($options));
	  
	 /** if ($options['name']=="update") {
   ...
   **/
   ...
   static function getCopyrightMessage($withVersion = true) {
      $message = "<a href=\"http://www.fotex.es/\" title=\"Powered by FOTEX/Teclib and contributors\" class=\"copyright\">";
      $message .= "FOTEX ";
      // if required, add GLPI version (eg not for login page)
      if ($withVersion) {
          $message .= GLPI_VERSION . " ";
      }
      $message .= "Copyright (C) 2015-" . GLPI_YEAR . " FOTEX/Teclib' and contributors".
    ...
    * @return void
    */
   static public function loadJavascript() {
      global $CFG_GLPI, $PLUGIN_HOOKS;
...
//        }    
   ...
   function proyectosUsuario($id){
       ...
       while ($data = $DB->fetch_array($result)) {       
           array_push($CFG_GLPI["project_user"] , $data['projects_id'] );
       }
   }
     ...
     /*echo"<pre>";
             ...
             if(($data['title']!='Administración') && ($data['title']!='Configuración')){
                if($data['title']=='Proyectos' && !isset($data['default'])){
                     unset($menu[$part]['content']);
                       ...
                       /** if ($data['title']!='Proyectos' || $data['title']!='Proveedores') {
                            echo "<li id='menu$i' data-id='$i' class='$menu_class' style='display: inline-flex;'>";
                        }else{
                            echo "<li id='menu$i' data-id='$i' class='$menu_class' style=''>";
                        }**/
                        if ($data['title']=='Proyectos' || $data['title']=='Proveedores') {
                        ...
                        if ($data['title']=='Proveedores') {
                            if(in_array($profile_Id, array(3,4,14,15,16))){
                                echo "<img id='imgProveedormnu' src='{$CFG_GLPI["root_doc"]}/pics/boton_proveedor_grande_inverso.png' style='margin-top: 4px; margin-left: 4px; height:20px;'/>";
                            }
                        }
                        if ($data['title']=='Proyectos') {
                            if(in_array($profile_Id, array(3,4,14,15,16))){
                                echo "<img id='imgProyectomnu' src='{$CFG_GLPI["root_doc"]}/pics/proyectos_inverso.png' style='margin-top: 4px; margin-left: 4px; height:20px;'/>";
                            }
                        }
                        /**
                        if ($data['title']=='Gestión' &&) {
                            echo "<img id='imgGestionmnu' src='{$CFG_GLPI["root_doc"]}/pics/rueda_dentada_inverso.png' style='margin-top: 4px; margin-left: 4px; height:20px;'/>";
                        }                        
                        if ($data['title']=='Herramientas') {
                            echo "<img id='imgHerramientamnu' src='{$CFG_GLPI["root_doc"]}/pics/herramienta_inverso.png' style='margin-top: 4px; margin-left: 4px; height:20px;'/>";
                        }           **/             
                        if ($data['title']=='Proyectos' || $data['title']=='Proveedores') {
                            if(in_array($profile_Id, array(3,4,14,15,16))){
                        ...
                        if (!isset($data['content'][0]) || $data['content'][0] !== true) {
                           if ($data['title']!='Proyectos' && $data['title']!='Proveedores') {
                               /**
                                echo "<ul class='ssmenu' style='margin-top: 30px;'>";
                                // list menu item
      ...
      if ($full === true) {
         ...
         if (isset($menu[$sector]['content'][$item])) {
            ...
            echo "<span>";
            if (isset($links['add'])) {
                if((in_array($profile_Id, array(3,4,16)) && $options['item']=='project')||((in_array($profile_Id, array(3,4,14,16)) && $options['item']!='project'))){
...
//            // Mis proyectos
//            if($options['item']=='project'){             
//                echo "<span id='misproyectos'>";            
//                    if (isset($links['search'])) {
//                       echo "<a href='{$CFG_GLPI['root_doc']}{$links['search']}' class='pointer'
//                                         title='Mis proyectos'><i class='boton_misproyectos'></i>
//                                         <span class='boton_misproyectos'></span></a>";
//                    } else {
//                       echo "<a href='#' class='pointer disabled' title='Mis proyectos está desactivado'>".
//                          "<i class='boton_misproyectos_dissabled'></i>".
//                          "<span class='boton_misproyectos_dissabled'></span></a>";
//                    }                
//                echo "</span>";            
//            }
            ...
            if($options['item']=='supplier'){ 
                echo"<span>";
                echo"<a href='#' class='pointer' onclick='cambiarBuscadorPreseleccion()' title='preselección de proveedores'><i class='preseleccion fa'></i></a>";
