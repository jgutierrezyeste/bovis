...
class Dropdown {
   ...
   **/
   ...
   static function checkboxList($itemtype, $options = []) {
      global $DB, $CFG_GLPI;
      ...
      //$sql = "SELECT ID, NAME FROM {$table} WHERE {$params['condition']} ORDER BY NAME ASC";
      $sql = "SELECT ID, NAME FROM glpi_plugin_comproveedores_specialties WHERE {$params['condition']} ORDER BY NAME ASC";
      if($result = $DB->query($sql)){
            $output = "<ul>";
            while ($data = $DB->fetch_array($result)) {
                $output .= "<li><input id='{$params['name']}_{$data['ID']}' type='checkbox' ";
                if($data['id']==$params['value']){
                    $output .= "value='1' />";
                }else{
                    $output .= "value='0' />";
                }
