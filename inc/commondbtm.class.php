...
class CommonDBTM extends CommonGLPI {
   ...
   **/
   function showDates($options = []) {
      ...
      if ($date_creation_exists && $this->getID() > 0  && (!isset($options['withtemplate']) || $options['withtemplate'] == 0)) {
         echo "<th style='font-weight: normal;'  colspan='$colspaninc'>";
      ...
      if ($date_mod_exists) {
         echo "<th style='font-weight: normal;' colspan='".($colspan * 4)."'>";
   ...
   **/
   function showFormButtons($options = []) {
      ...
      if ($params['withtemplate'] ||$this->isNewID($ID)) {
         ...
         echo "<td class='center' colspan='".($params['colspan']*2)."' style='border-top: 1px solid #e5e5e5;'>";
         ...
         if ($params['canedit'] && $this->can($ID, UPDATE)) {
            echo "<td class='center' colspan='".($params['colspan'])."' style='border-top: 1px solid #e5e5e5;'>\n";
            echo Html::submit(_x('button', 'Save'), ['name' => 'update']);
         }
         ...
         if ($params['candel']) {
            ...
            if ($this->isDeleted()) {
               if ($this->can($ID, DELETE)) {
                  echo "<td class='right' colspan='".($params['colspan'])."' style='border-top: 1px solid #e5e5e5;'>\n";
               ...
               echo "<td class='left' colspan='".($params['colspan']*2)."' style='border-top: 1px solid #e5e5e5;'>\n";
      ...
      if ($params['canedit'] && count($params['addbuttons'])) {
         echo "<tr class='tab_bg_2'>";
         if ((($params['colspan']) - count($params['addbuttons'])) > 0) {
            echo "<td colspan='".($params['colspan'] - count($params['addbuttons']))."'>&nbsp;</td>";
         }
         foreach ($params['addbuttons'] as $key => $val) {
            echo "<td><input class='boton_grabar' type='submit' name='$key' value=\"".Html::entities_deep($val)."\" /></td>";
   ...
   **/
   function showFormHeader($options = []) {
      ...
      if($params['cv'] == false){
            echo "<div class='spaced' id='tabsbody' style='overflow-y: hidden;'>";
            echo "<table class='tab_cadre_fixe' id='mainformtable' style='margin-top:10px !important;'>";
      }else{
            echo "<div class='spaced' id='tabsbody' style='width: 100%; float: left; position: relative; max-height: 500px; overflow-y: scroll; background-color: #e9ecf3; padding: 5px;'>";
            echo "<table id='mainformtable' style='float: left; position: relative; margin-top:10px !important; width: 100%;'>";  
      }
      
      
      
      if ($params['formtitle'] !== '' && $params['formtitle'] !== false) {
         echo "<tr class='headerRow' style='visibility: collapse;'><th colspan='".$params['colspan']."'>";
