...
(select GROUP_CONCAT(distinct items_id SEPARATOR '<BR>') 
...
//comprobamos que se a enviado algún filtro de busqueda
    //DURACIÓN
    $where = " proyectos.is_deleted=0 AND (TIMESTAMPDIFF(MONTH, plan_start_date, plan_end_date)<={$_GET['duracionMaxima']} AND TIMESTAMPDIFF(MONTH, plan_start_date, plan_end_date)>={$_GET['duracionMinima']}) ";
...
$result = $DB->query($query);
	...
	//echo "<div class='cuadro_busqueda' style='width: 80%; padding: 4px;'>";
	//echo "";			
	//echo "<table class='tab_cadre_fixehov' style='margin:4px auto; width:80%;' >";
	...
	while ($data=$DB->fetch_array($result)) {
            if(in_array($profileID, array(3, 4, 14, 15,16)) ){ //si el perfil es usuario de proyecto y el usuario pertenece al equipo entonces de visualiza
                echo "<tr>";
                    //echo "<td><input id='proyecto_".$data["id"]."' onclick=añadirProyectoIdPDF(".$data["id"].") type='checkbox' /></td>";
                    if($profileID == 15){
                        echo "<td style='text-align:left;'>".$data["name"]."</td>";               
                    }else{
                        if($profileID == 14 && in_array($data['id'], $prj) == false){
                            echo "<td style='text-align:left;'>".$data["name"]."</td>";               
                        }else{
