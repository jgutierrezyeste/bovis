 ...
 ******************************************/
    ...
    // Only allowed types
			...
			return $types;
                ...
                function frmMantenimientoFinanciero($CVID = 0){
                    GLOBAL $DB,$CFG_GLPI;
                    
                        echo "<table>";
                        echo "<tr>";
                        echo "<td>año</td><td>facturación</td><td>beneficios ai</td><td>resultado</td><td>total activo</td><td>activo circulante</td>";
                        echo "</tr>";
                        echo "<tr>";
                        echo "<td><input id='anio' type='text' value='0'/></td><td><input id='facturacion' type='text' value='0'/></td><td><input id='beneficios_impuestos' type='text' value='0'/></td><td><input id='resultado' type='text' value='0'/></td><td><input id='total_activo' type='text' value='0'/></td><td><input id='activo_circulante' type='text' value='0'/></td>";
                        echo "</tr>";
                        echo "</table>";                    
                        
                }
                
                function registroFinanciero($CVID = 0){
                    GLOBAL $DB,$CFG_GLPI;

                    echo "<style class='text/css'>.contenedorRegistro{
                        border-radius: 4px;
                        font-size: 14px;
                        width: 100%;
                    }</style>";
                    $sql = "SELECT id,
                            ...
                            ORDER BY anio DESC";
                    $result = $DB->query($sql);
                    echo "<div class='contenedorRegistro'>";
                    $tabla ="<table style='font-size: 11px; border: none;'>";
                    $tabla.="<thead><tr><th>año</th><th>facturación</th><th>beneficios ai</th><th>resultado</th><th>total activo</th><th>activo circ.</th><th>pasivo circ.</th><th>cash flow</th><th>fondos prop</th><th>recursos ajenos</th></tr></thead>";
                    $tabla.="<tbody>";
                    while ($r=$DB->fetch_array($result)) {                    
                        $tabla.="<tr id='annualbillingsid_{$r['id']}'>";
                        $tabla.="<td>{$number_format($r['anio'],0,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['facturacion'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['benecificios_impuestos'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['total_activo'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['activo_circulante'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['pasivo_circulante'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['cash_flow'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['fondos_propios'],2,',','.')}</td>";
                        $tabla.="<td>{$number_format($r['recursos_ajenos'],2,',','.')}</td>";
                        $tabla.="</tr>";
                    }
                    $tabla .= "</tbody></table>";
                    echo $tabla;
		...
		function showFormItemFinancial($item, $withtemplate='') {	
			...
			echo "</tr>";
                        ...
                        echo "<div id='cuatro' class='center' style='background-color: #e6e6e6; padding: 5px; border: 1px solid #ccc; border-radius: 4px; width: 70%; margin-left:18px; height: 320px; float: left; position: relative;'>";
                            ...
                            while ($data=$DB->fetch_array($result2)) {
                                echo"<tr id='idannualbilings_".$data['id']."' >";
                                        ...
                                        if($data['fondos_propios']!=''){
                                            echo number_format($data['fondos_propios'],2,',','.')."<br>";
                            ...
                            $obl = 0;
                            $prc = 0;
                            $qAvg = "select avg(fondos_propios) as fondos_propios,
                                    avg(100*recursos_ajenos/(fondos_propios+recursos_ajenos)) as apalancamiento,
                                    avg((100*beneficios_impuestos/facturacion)) as margen_beneficio,
                                    avg(100*cash_flow/facturacion) as efectivo_sobre_facturacion,
                                    avg(activo_circulante/pasivo_circulante) as situacion_actual_liquidez,
                                    avg(100*beneficios_impuestos/fondos_propios) as roce
                                            from glpi_plugin_comproveedores_annualbillings 
                                            where cv_id=".$CvId." order by anio desc limit 3";
                            $rAvg = $DB->query($qAvg);   
                            //echo $qAvg;
                            echo "<tfoot>";
                                echo "<tr style=''>";
                                    while ($dAvg=$DB->fetch_array($rAvg)) {       
                                        echo "<td class='right' colspan='11' style='font-weight: bold; border: 1px solid #e6e6e6;'>Media de los 3 últimos ejercicios -></td>";                             
                                        echo "<td style='font-weight: bold; border: 1px solid #e6e6e6;'>".number_format($dAvg['fondos_propios'],2,',','.')."€<br>";
                                            if($dAvg['fondos_propios']>=$fondos_propios){$obl++; echo "<img src='".$CFG_GLPI["root_doc"]."/pics/si.png' style='' />";}
                                            else{echo "<img src='".$CFG_GLPI["root_doc"]."/pics/no.png' style='' />";}                                        
                                        echo "</td>";
                                        echo "<td style='font-weight: bold; border: 1px solid #e6e6e6;'>".number_format($dAvg['apalancamiento'],2,',','.')."%<br>";
                                            if($dAvg['apalancamiento']<$apalancamiento){$obl++; echo "<img src='".$CFG_GLPI["root_doc"]."/pics/si.png' style='' />";}
                                            else{echo "<img src='".$CFG_GLPI["root_doc"]."/pics/no.png' style='' />";}                                            
                                        echo "</td>";
                                        echo "<td style='font-weight: bold; border: 1px solid #e6e6e6;'>".number_format($dAvg['margen_beneficio'],2,',','.')."%<br>";
                                            if($dAvg['margen_beneficio']>$margen_beneficio){$prc++; echo "<img src='".$CFG_GLPI["root_doc"]."/pics/si.png' style='' />";}
                                            else{echo "<img src='".$CFG_GLPI["root_doc"]."/pics/no.png' style='' />";}                                               
                                        echo "</td>";
                                        echo "<td style='font-weight: bold; border: 1px solid #e6e6e6;'>".number_format($dAvg['efectivo_sobre_facturacion'],2,',','.')."%<br>";
                                            if($dAvg['efectivo_sobre_facturacion']>$efectivo_facturacion){$prc++; echo "<img src='".$CFG_GLPI["root_doc"]."/pics/si.png' style='' />";}
                                            else{echo "<img src='".$CFG_GLPI["root_doc"]."/pics/no.png' style='' />";}                                               
                                        echo "</td>";
                                        echo "<td style='font-weight: bold; border: 1px solid #e6e6e6;'>".number_format($dAvg['situacion_actual_liquidez'],2,',','.')."<br>";
                                            if($dAvg['situacion_actual_liquidez']>$situacion_liquidez){$prc++; echo "<img src='".$CFG_GLPI["root_doc"]."/pics/si.png' style='' />";}
                                            else{echo "<img src='".$CFG_GLPI["root_doc"]."/pics/no.png' style='' />";}                                               
                                        echo "</td>";
                                        echo "<td style='font-weight: bold; border: 1px solid #e6e6e6;'>".number_format($dAvg['roce'],2,',','.')."%<br>";
                                            if($dAvg['roce']>$roce){$porc++; echo "<img src='".$CFG_GLPI["root_doc"]."/pics/si.png' style='' />";}
                                            else{echo "<img src='".$CFG_GLPI["root_doc"]."/pics/no.png' style='' />";}                                               
                                        echo "</td>";
                                    }
                                echo "</tr>";
			...
			echo "</div>";
                        ...
                        echo "<p style='position: relative; float: left; width: 98%; 
                            ...
                            (*) Todos los datos están en miles de €.<br>
...
//window.location.reload(true);
		...
		function showFormItem($item, $withtemplate='') {	
			...
			//$PluginCv->initForm($CvId, $options);
			//$PluginCv->showFormHeader($options);
			echo "<div id='contenedorFinancial' style='margin-bottom: 10px; float:left; width:98%; border-radius: 4px; padding: 8px; background-color: #e9ecf3;'>";
			echo Html::hidden('id', array('value' => $CvId));
			...
			echo Html::hidden('_glpi_csrf_token', array('value' => Session::getNewCSRFToken()));
                        ...
                        echo "<table class='tab_cadre_fixe' style='width: 100%; margin-top: 4px; margin-bottom: 10px; padding: 5px; border-radius:4px; background-color:#f8f7f3; -webkit-box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);-moz-box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);'>";
			echo "<tr style='font-weight: bold; font-size:12px; '>";
                            echo "<td class='right'>CAPITAL SOCIAL:</td>";
                            echo"<td class='left'>";
                            Html::autocompletionTextField($this, "capital_social", array('value'=>$capital_social));
			...
			echo "</tr>";
                        ...
                        echo "<div id='areaTrabajoFinancial' style='margin-left:5px; width: 95%; float: left; position: relative;'>";
                        echo "<div class='center' style='margin-left:5px; width: 21%; float: left; position: relative; '>";
                            echo "<div class='center' style='-webkit-box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);  -moz-box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);    box-shadow: 12px 8px 5px -5px rgba(219,219,219,1); background-color: #fff; padding: 5px; border: 1px solid #ccc; border-radius: 4px; margin-top: 0px; margin-left:5px; width: 298px; height: 300px; float: left; position: relative;'>";
                                $query3 ="SELECT obl_fondos_propios, "
                                        . "obl_apalancamiento*100 as obl_apalancamiento,"
                                        . "parcial_margen_beneficio*100 as parcial_margen_beneficio,"
                                        . "parcial_efectivo_facturacion*100 as parcial_efectivo_facturacion,"
                                ...
                                while ($d=$DB->fetch_array($result3)) {
                                    ...
                                    echo "<tr><td class='right' style='padding: 4px; background-color:#f7edcf; border: 1px solid #f7edcf;'>Sitiación liquidez:</td><td style='padding: 4px; background-color:#f7edcf; border: 1px solid #f7edcf;'>>".number_format($d['parcial_situacion_liquidez'],2,',','.')."%</td></tr>";
                                    echo "<tr><td class='right' style='padding: 4px; background-color:#f7edcf; border: 1px solid #f7edcf;'>ROCE:</td><td style='padding: 4px; background-color:#f7edcf; border: 1px solid #f7edcf;'>".number_format($d['parcial_roce'],2,',','.')."%</td></tr>";                                
                                    echo "<tr><td colspan='2' class='right' style='font-weight: bold; padding-top: 20px; font-size:14px;'>Anualidad media según facturación anual</td><td style='padding-top: 20px; font-weight: bold; font-size:14px;'>>".number_format($d['anualidad_media_segun_facturacion_anual'],2,',','.')."%</td></tr>";
                                }            
                                echo "</table>";
                            echo "</div>";
                        ...
                        echo "</div>";
                        echo "</div>";
                        echo "<p style='position: relative; float: left; width: 99%; font-size: 10px; margin-top: 4px; padding: 5px; border-radius: 4px; background-color: #f8f7f3; -webkit-box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);    -moz-box-shadow: 12px 8px 5px -5px rgba(219,219,219,1); box-shadow: 12px 8px 5px -5px rgba(219,219,219,1);'>(*) Todos los datos están en miles de €. <br>(**) Aunque se diponga de mas anualidades, los cálculos de las medias se realizarán con los 3 últimos ejercicios.<br> (***) El cumplimiento con los criterios se realizan sobre el último año que se disponga información. </p>";
